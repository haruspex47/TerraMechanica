## Касаемо всего
Во-первых, общая структура конспектов такая: в общей папке хранится pdf файл вёрстки $\LaTeX$, внутри которого всё, что есть (по мере внесения мною изменений, конечно). Сам файл $\TeX$ в гитигноре, но если кто-то хочет посмотреть или участвовать, то могу открыть. В остальном структура проста, всего две с половиной части (введение, кинематика, статика), внутри разделение на главы, дальше на коyспекты, где находится два .md (markdown, см. далее) файла — сам конспект (теоретическая часть) и примеры (практическая часть). Там же папка с картинками, на которые мы ссылаемся. Не удивляйтесь странным названиям папок (без намёка на содержимое) и не пытайтесь их, пожалуйста, сменить без моего ведома — так, к сожалению, надо. Может потом проблема решится. То же касается и соблазна создать новый раздел (вероятно, параграф): сначала лучше напишите мне. Впрочем, здесь уже не так критично.
...
К сожалению, при работе с матджаксом иногда возникают сложности, которые не должны возникать :hurtrealbad:. К примеру, перед и после `$$` вроде как (!) нужно пропускать строку. Из-за подобных мелочей у вас вместо формулы будет её код. Аналогичную проблему мы получим, использовав в формуле некорректную команду, так что "отладка" в матджаксе сумеет доставить неприятности.  
...
Ещё один частный случай возникает с угловыми скобками и двоеточием. Что же особенного в угловых скобках и двоеточии? Разгадка проста: символы `<:>` внутри формулы понимаются как математические знаки сравнения и отношения, и форматируются они с отступами с обеих сторон. Если же мы хотим пользоваться ими как скобками и двоеточием[^4], то __обязательно__ заменяем их на команды `\langle \colon \rangle`. 
...
[^4]: Двоеточие используется реже. В формуле на языке логики оно означает "такой, что".
[^5]: можете ещё на номо$\TeX$е подглядывать ахахаха (не относитесь к этим словам серьёзно)
