...
К сожалению, при работе с матджаксом иногда возникают сложности, которые не должны возникать :hurtrealbad:. К примеру, перед и после `$$` вроде как (!) нужно пропускать пустую строку[^6]. Или при написании формулы вроде как (!) после доллара нельзя делать пробел, то есть `$ формула$` не сработает. Из-за подобных мелочей у вас вместо формулы будет отображаться её код. Ровно ту же проблему мы получим, использовав в формуле некорректную команду, или допустив любую другую ошибку, так что "отладка" в матджаксе сумеет доставить неприятности.  
