...
$$x+y=4. \label{eq:example1}$$ Если верить формуле <a href="#eq:example1" data-reference-type="eqref" data-reference="eq:example1">[eq:example1]</a>, то $x=y=2$.

### Mathjax
Mathjax позволяет использовать в маркдауне формулы с помощью системы вёрстки $\TeX$ (а точнее $\LaTeX$ с некоторыми дополнительными пакетами (а точнее урезанной очень сильно урезанной его версии с некоторыми дополнительными возможностями)). Писать формулы в $\LaTeX$е на том уровне, на котором требуется сейчас, достаточно просто. Гораздо проще, во всяком случае, чем в ворде. 
